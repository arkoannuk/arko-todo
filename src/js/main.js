...
class Project {
  ...
  addTodo(title, desc, date, starred, status, projectIndex) {
    ...
    const todoIndex = this.todoList.length - 1 // This is being calculated wrong since the this.todoList will be sorted | Should've used IDs from the start (TODO: Optimize)
    this.todoList = this.sortTodoList(this.todoList)
    this.displayController.addTodoCard(
      title,
      desc,
      date,
      starred,
  ...
  editTodo(title, desc, date, starred, todoIndex, projectIndex) {
    ...
    this.displayController.editTodoCard(
      title,
      desc,
      date,
      starred,
...
class DisplayController {
  hideTodoCard(checked, todoIndex, projectIndex) {
    const todoCard = document.querySelector(
      `[data-project-index="${projectIndex}"][data-todo-index="${todoIndex}"]`
    )
  ...
  deleteTodoCard(todoCard) {
    const projectIndex = todoCard.dataset.projectIndex
    todoCard.remove()
    const remainingTodoCards = document.querySelectorAll(
      `[data-project-index="${projectIndex}"]`
    )
    remainingTodoCards.forEach(
      (card, index) => (card.dataset.todoIndex = index)
    )
  }

  deleteProjectSelector() {
    const projectSelector = document.getElementById('projectSelector')
    const deletableProject = projectSelector.value
    const optionToRemove = projectSelector.querySelector(
      `option[value="${deletableProject}"]`
    )
    projectSelector.removeChild(optionToRemove)

    const options = projectSelector.getElementsByTagName('option')
    for (let i = 0; i < options.length; i++) {
      // Update remaining selector values
      options[i].value = i
    }
  }
  ...
  showSelectedProject() {
    ...
    if (showAll) {
      ...
      const selectedProject = storedProjects[projectIndex]
      const projectName = document.createElement('h3')
      projectName.classList.add('mt-2', 'text-center', 'text-secondary')
      ...
      document.getElementById('todoList').appendChild(projectName)
      selectedProject
        .sortTodoList(selectedProject.todoList)
        .forEach((todo, index) => {
          this.displayController.addTodoCard(
            todo.title,
            todo.desc,
    ...
    const DoneTodoCards = Array.from(todoCards).filter((todoCard) => {
      const status = todoCard.querySelector('input.todoStatusBtn')
  ...
  addProjectSelector(name, index) {
    const projectSelectorTemp = document
      .getElementById('projectSelectorTemp')
      .content.cloneNode(true)
    const projectSelectorOption = projectSelectorTemp.querySelector(
  ...
  addTodoCard(
    ...
    const todoCardTemp = document
      .getElementById('todoCardTemp')
      .content.cloneNode(true)
    ...
    if (newCard) {
      // This is deprecated due to refresh happening after every add
  ...
  showEditModal(todoCard) {
    const todoTitle = todoCard.querySelector('.todoTitle').textContent
    const todoDesc = todoCard.querySelector('.todoDesc').textContent
    const todoDate = todoCard.querySelector('.todoDate').textContent
    const todoStarred = todoCard.querySelector('.todoStarred')

    const editTodoFormModal = document.getElementById('editTodoFormModal')
...
class App {
  constructor() {
    ...
    document
      ...
      .addEventListener('submit', this.submitTodoForm.bind(this))
    document
      .getElementById('deleteProject')
      .addEventListener('click', this.deleteProject.bind(this))
    document
      .getElementById('deleteProject')
      .addEventListener(
        'click',
        this.displayController.showSelectedProject.bind(this)
      )
    document
      .getElementById('projectForm')
      .addEventListener('submit', this.submitProjectForm.bind(this))
    document
      .getElementById('projectForm')
      .addEventListener(
        'submit',
    ...
    document
      ...
      .addEventListener('submit', this.submitEditTodoForm.bind(this))
  ...
  changeTodoStatus(todoStatusBtn, todoCard) {
    ...
    const projectIndex = todoCard.dataset.projectIndex
    const todoIndex = todoCard.dataset.todoIndex
    const checked = todoStatusBtn.checked
  ...
  addProject(name) {
    ...
    localStorage.setItem('appJSON', JSON.stringify(storedProjects))
    const index = storedProjects.length - 1
  ...
  submitEditTodoForm(event) {
    ...
    const editTodoFormModal = document.getElementById('editTodoFormModal')
    const projectIndex = editTodoFormModal.dataset.projectIndex
    const todoIndex = editTodoFormModal.dataset.todoIndex

    const title = document.getElementById('todoEditTitle').value
    const desc = document.getElementById('todoEditDesc').value
    const date = document.getElementById('todoEditDate').value

    const todoStarredCheckbox = document.querySelector('#todoEditStarred')
    const starred = todoStarredCheckbox.checked

    storedProjects[projectIndex].editTodo(
      title,
      desc,
      date,
      starred,
  ...
  submitTodoForm(event) {
    ...
    storedProjects[projectIndex].addTodo(
      title,
      desc,
      date,
      starred,
  ...
  createSampleData() {
    ...
    if (!localStorage.getItem('appJSON')) {
      ...
      this.projectList[0].addTodo(
        'Buy Groceries',
        'Pick up milk, eggs, bread, and vegetables on the way home from work.',
        '2023-09-12',
        true,
        false,
        0
      ...
      this.projectList[0].addTodo(
        'Plan Vacation',
        'Research and plan a family vacation to the beach for next summer.',
        '',
        false,
        false,
        0
      ...
      this.projectList[0].addTodo(
        'Water Plants',
        'Weekly watering required for my living room plants.',
        '',
        true,
        true,
        0
      )
      this.projectList[1].addTodo(
        'Prepare Presentation',
        'Create slides and outline for the upcoming client presentation.',
        '2023-09-25',
        false,
        false,
        1
      )
      this.projectList[2].addTodo(
        'Start Exercise Routine',
        'Start a new workout routine, including cardio and strength training.',
        '2023-09-15',
        true,
        false,
        2
      ...
      this.projectList[2].addTodo(
        'Plan Healthier Diet',
        'Research and gather a list of healthy recipes for next week.',
        '2023-09-10',
        false,
        false,
        2
